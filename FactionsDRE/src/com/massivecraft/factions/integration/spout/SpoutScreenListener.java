...
public class SpoutScreenListener implements Listener {
    ...
    public void onButtonClick(ButtonClickEvent event) {
        ...
        if(Button==player.getsMenu().faction_membersbutton){
        	player.getsMenu().faction_membersbox.setVisible(true);player.getsMenu().faction_membersbutton.setEnabled(false);
        	player.getsMenu().faction_alliesbox.setVisible(false);player.getsMenu().faction_alliesbutton.setEnabled(true);
        	player.getsMenu().faction_enemiesbox.setVisible(false);player.getsMenu().faction_enemiesbutton.setEnabled(true);
        }
        
        if(Button==player.getsMenu().faction_alliesbutton){
        	player.getsMenu().faction_membersbox.setVisible(false);player.getsMenu().faction_membersbutton.setEnabled(true);
        	player.getsMenu().faction_alliesbox.setVisible(true);player.getsMenu().faction_alliesbutton.setEnabled(false);
        	player.getsMenu().faction_enemiesbox.setVisible(false);player.getsMenu().faction_enemiesbutton.setEnabled(true);
        }
        ...
        if(Button==player.getsMenu().faction_enemiesbutton){
        	player.getsMenu().faction_membersbox.setVisible(false);player.getsMenu().faction_membersbutton.setEnabled(true);
        	player.getsMenu().faction_alliesbox.setVisible(false);player.getsMenu().faction_alliesbutton.setEnabled(true);
        	player.getsMenu().faction_enemiesbox.setVisible(true);player.getsMenu().faction_enemiesbutton.setEnabled(false);
        }
    ...
    private void createFaction(FPlayer fme, String tag){
		...
		if ( ! fme.payForCommand(Conf.econCostCreate, "to create a new faction", "for creating a new faction")) return;
