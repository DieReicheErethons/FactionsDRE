...
public class EssentialsFeatures {
	...
	public static void setup() {
		...
		// TODO: this is the old Essentials method not supported in 3.0...
		// probably needs to eventually be moved to EssentialsOldVersionFeatures
	...
	public static void integrateChat(EssentialsChat instance) {
		...
		try {
			...
			// curly braces used to be accepted by the format string
			// EssentialsChat but no longer are, so... deal with
			// chatTagReplaceString which might need updating
	...
	private static class LocalChatListener implements Listener {
		...
		public void onPlayerChat(EssentialsLocalChatEvent event) {
			...
			// NOTE: above doesn't do relation coloring. if/when we can get a
			// local recipient list from EssentialsLocalChatEvent, we'll
			// probably
			// want to pass it on to
			// FactionsPlayerListener.onPlayerChat(PlayerChatEvent event) rather
