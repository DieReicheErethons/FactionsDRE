...
public abstract class FCommand extends MCommand<P> {
	...
	public boolean payForCommand(double cost, String toDoThis, String forDoingThis) {
		...
		if (Conf.bankEnabled && Conf.bankFactionPaysCosts && fme.hasFaction()) {
			if (!Econ.modifyMoney(myFaction, -cost, toDoThis, forDoingThis))
				return false;
		} else {
			if (!Econ.modifyMoney(fme, -cost, toDoThis, forDoingThis))
				return false;
		}
		return true;
		/*
		 ...
		 * 
		 * 
		 * // pay up if (cost > 0.0) { String costString =
		 * Econ.moneyString(cost); if(Conf.bankFactionPaysCosts &&
		 * fme.hasFaction() ) { if( ! faction.getAccount().subtract(cost)) {
		 * sendMessage("It costs "+costString+" to "+desc+
		 * ", which your faction can't currently afford."); return false; } else
		 * {
		 * sendMessage(faction.getTag()+" has paid "+costString+" to "+desc+"."
		 * ); }
		 * 
		 * } else { if ( ! Econ.deductMoney(fme.getName(), cost)) {
		 * sendMessage("It costs "
		 * +costString+" to "+desc+", which you can't currently afford.");
		 * return false; }
		 * sendMessage("You have paid "+costString+" to "+desc+"."); } } //
		 * wait... we pay you to use this command? else { String costString =
		 * Econ.moneyString(-cost);
		 * 
		 * if(Conf.bankFactionPaysCosts && fme.hasFaction() ) {
		 * faction.getAccount().add(-cost);
		 * sendMessage(faction.getTag()+" has been paid "
		 * +costString+" to "+desc+"."); } else { Econ.addMoney(fme.getName(),
		 * -cost); }
		 * 
		 * 
