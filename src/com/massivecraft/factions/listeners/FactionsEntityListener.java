...
public class FactionsEntityListener implements Listener {
	...
	public void onEntityExplode(EntityExplodeEvent event) {
		...
		if (event.getEntity() instanceof Creeper
				&& ((faction.isNone() && Conf.wildernessBlockCreepers && !Conf.worldsNoWildernessProtection.contains(loc.getWorld().getName()))
						|| (faction.isNormal() && (online ? Conf.territoryBlockCreepers : Conf.territoryBlockCreepersWhenOffline)) || (faction.isWarZone() && Conf.warZoneBlockCreepers) || faction
							.isSafeZone())) {
			...
			event.setCancelled(true);
				...
				&& ((faction.isNone() && Conf.wildernessBlockFireballs && !Conf.worldsNoWildernessProtection.contains(loc.getWorld().getName()))
						|| (faction.isNormal() && (online ? Conf.territoryBlockFireballs : Conf.territoryBlockFireballsWhenOffline)) || (faction.isWarZone() && Conf.warZoneBlockFireballs) || faction
							.isSafeZone())) {
			// ghast fireball which needs prevention
			event.setCancelled(true);
		} else if ((faction.isNone() && Conf.wildernessBlockTNT && !Conf.worldsNoWildernessProtection.contains(loc.getWorld().getName()))
				|| (faction.isNormal() && (online ? Conf.territoryBlockTNT : Conf.territoryBlockTNTWhenOffline)) || (faction.isWarZone() && Conf.warZoneBlockTNT)
	...
	private static final Set<PotionEffectType> badPotionEffects = new LinkedHashSet<PotionEffectType>(Arrays.asList(PotionEffectType.BLINDNESS, PotionEffectType.CONFUSION, PotionEffectType.HARM,
			PotionEffectType.HUNGER, PotionEffectType.POISON, PotionEffectType.SLOW, PotionEffectType.SLOW_DIGGING, PotionEffectType.WEAKNESS));
	...
	/**
	 * Canceled redstone torch placement next to existing TNT is still
	 * triggering an explosion, thus, our workaround here. related to this:
	 * https://bukkit.atlassian.net/browse/BUKKIT-89 though they do finally
	 * appear to have fixed the converse situation (existing redstone torch, TNT
	 * placement attempted but canceled)
	 */
	...
	public void onExplosionPrime(ExplosionPrimeEvent event) {
		...
		for (int i = exploitExplosions.size() - 1; i >= 0; i--) {
			...
			if (absX < 5 && absZ < 5) { // it sure looks like an exploit attempt
