...
public class Factions extends EntityCollection<Faction>
{
	...
	public boolean loadFromDisc()
	{
		...
		// Make sure the NeutralZone faction exists
		if ( ! this.exists("-3"))
		{
			Faction faction = this.create("-3");
			faction.setTag("NeutralZone");
			faction.setDescription("Eroberbares Gebiet");
		}
		else
		{
			// if NeutralZone has old pre-1.6.0 name, rename it to remove troublesome " "
			Faction faction = this.getNeutralZone();
			if (faction.getTag().contains(" "))
				faction.setTag("NeutralZone");
		}
	...
	//----------------------------------------------//
	public void updateBeginnersProtection(){
		for(Faction faction:i.get()){
			if(faction.getBeginnerProtection()){
				Long sys=System.currentTimeMillis();
				
				if(sys>faction.getBeginnerProtectionTime()){
					faction.setBeginnerProtection(false);
				}
			}
		}
	}
